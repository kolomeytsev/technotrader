...
class Trader:
    def __init__(self, waiting_period, config, total_steps, net_dir, agent=None, initial_BTC=1.0, agent_type="nn"):
        ...
        prediction_range = SECONDS_TO_RES[config["input"]["global_period"]]
        last_candles_params = get_last_candles_params(
            prediction_range,
            config["input"]["market"],
            config["input"]["features"],
            config["input"]["coins_list"]
        )
        if agent_type == "traditional":
            ...
            self._rolling_trainer = RollingTrainer(config, last_candles_params, net_dir, agent=agent)
