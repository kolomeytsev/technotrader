...
import numpy as np
from rllib.trade import trader
from rllib.marketdata.datamatrices import DataMatrices
import logging
from rllib.tools.trade import calculate_pv_after_commission
...
class BackTest(trader.Trader):
    def __init__(self, config, net_dir=None, agent=None, agent_type="nn"):
        ...
        if agent_type == "nn":
            ...
            config["input"]["features"] = ["close"]
            last_candles_params = (
                "binance",
                config["input"]["features"],
                SECONDS_TO_RES[config["input"]["global_period"]],
                None
            )
            data_matrices = DataMatrices.create_from_config(config, last_candles_params)
        else:
            raise ValueError()
        self.data_matrices = data_matrices
        self.__test_set = data_matrices.get_test_set()
        self.test_dates = data_matrices.get_test_dates()
        self.__test_length = self.__test_set["X"].shape[0]
        self._total_steps = self.__test_length
        self.__test_pv = 1.0
        self.__test_pc_vector = []
        self.__test_pc_vector_no_commission = []
        self.weights_list = []
    ...
    @property
    def test_pc_vector_no_commission(self):
        return np.array(self.__test_pc_vector_no_commission, dtype=np.float32)
    ...
    def trade_by_strategy(self, omega):
        if self._steps % self.log_frequency == 0:
        ...
        portfolio_change_no_commission = np.dot(omega, future_price)
        ...
        self.__test_pc_vector_no_commission.append(portfolio_change_no_commission)
        self.weights_list.append(omega)
