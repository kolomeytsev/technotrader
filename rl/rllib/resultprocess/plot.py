...
import numpy as np
import datetime
from rllib.tools.indicator import max_drawdown, sharpe, positive_count, negative_count, moving_accumulate
...
def plot_backtest(config, algos, labels=None):
    ...
    for i, algo in enumerate(algos):
        if algo.isdigit():
            ...
            res, res_no_fee = execute_backtest(algo, config)
            results.append(np.cumprod(res))
...
def execute_strategies(config, algos, labels=None):
    """
    @:param config: config dictionary
    @:param algos: list of strings representing the name of algorithms or index of pgportfolio result
    """
    ...
    for i, algo in enumerate(algos):
        ...
        t0 = time.time()
        res, res_no_fee = execute_backtest(algo, config)
        results[algo] = (res, res_no_fee)
        logging.info("finish executing " + algo + ", time of execution: " + str(time.time() - t0))
        print()
    return results
...
def table_backtest(config, algos, labels=None, format="raw",
    ...
    for i, algo in enumerate(algos):
        if algo.isdigit():
            ...
            logging.info("start executing " + algo)
            portfolio_changes, portfolio_changes_no_fee = execute_backtest(algo, config)
            logging.info("finish executing " + algo)
