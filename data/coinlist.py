...
class CoinList(object):
    def __init__(self, end, volume_average_days=1, volume_forward=0):
        ...
        instruments = [
            'ETH', 'XRP', 'EOS', 'DASH', 'LTC', 'XMR', 'ETC', 'ARDR', 'BAT', 'ZEC'
        ]
        #instruments = {
        #    'ETH', 'XRP', 'DASH', 'LTC', 'XMR', 'ETC', 'ARDR', 'ZEC'
        #}
        self._df = self._df[self._df.index.isin(instruments)]
        print(self._df)
        assert self._df.shape[0] == len(instruments)
