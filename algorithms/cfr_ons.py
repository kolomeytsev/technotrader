...
class CfrOnsAgent(Agent):
    """
    Combination Forecasting Reversion Strategy (Huang et. al. June 2018).
    This version is based on Online Newton Step.
    ...
    def cfr_ons_next_price_relatives(self, prices):
        ...
        loss_grad = loss_grad.reshape(1, len(loss_grad))

        self.A += loss_grad.T @ loss_grad
        B = (np.linalg.inv(self.A) @ loss_grad.T).flatten()
        self.estimators_weights -= 1 / self.learning_rate * B
    ...
    def predict_next_weight(self, prices):
        if prices.shape[0] < self.window + 1:
            ...
            next_price_relative = self.cfr_ons_next_price_relatives(prices)
    ...
    def compute_portfolio(self, epoch):
        ...
        print("cfr-ons weights:", day_weight)
